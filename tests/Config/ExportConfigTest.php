...
final class ExportConfigTest extends TestCase
{
    ...
    public function testCompactFactoryMethod(): void { self::assertSame(FormatterMode::Standard, ExportConfig::compact()->mode); }

    public function testCopyMethodsPreserveOtherOptions(): void
    {
        $config = new ExportConfig(mode: FormatterMode::Pretty, indent: '  ', maxDepth: 12, sortKeys: true, trailingComma: true, closureUseMode: ClosureUseMode::Preserve, allowGenericReadonlyObjects: true, closureExportPolicy: ClosureExportPolicy::PortableExpression, sourcePathPolicy: SourcePathPolicy::Reject);
        ...
        self::assertSame(ClosureExportPolicy::SourceBound, $config->withClosureExportPolicy(ClosureExportPolicy::SourceBound)->closureExportPolicy);
        self::assertSame(SourcePathPolicy::AbsoluteBuildPath, $config->withSourcePathPolicy(SourcePathPolicy::AbsoluteBuildPath)->sourcePathPolicy);
        self::assertSame(FormatterMode::Standard, $config->withMode(FormatterMode::Standard)->mode);
    }

    #[DataProvider('invalidIndentProvider')]
    public function testThrowsForInvalidIndent(string $indent): void { $this->expectException(ConfigurationException::class); new ExportConfig(indent: $indent); }
    public static function invalidIndentProvider(): iterable { yield 'empty' => ['']; yield 'text' => ['abc']; yield 'two tabs' => ["\t\t"]; yield 'mixed whitespace' => [" \t "]; yield 'space then text' => ['  x']; }

    #[DataProvider('validIndentProvider')]
    public function testAcceptsPrinterCompatibleIndent(string $indent): void { self::assertSame($indent, (new ExportConfig(indent: $indent))->indent); }
    public static function validIndentProvider(): iterable { yield 'one space' => [' ']; yield 'two spaces' => ['  ']; yield 'four spaces' => ['    ']; yield 'tab' => ["\t"]; }

    #[DataProvider('invalidMaxDepthProvider')]
    public function testThrowsForInvalidMaxDepth(int $maxDepth): void { $this->expectException(ConfigurationException::class); new ExportConfig(maxDepth: $maxDepth); }
    public static function invalidMaxDepthProvider(): iterable { yield 'zero' => [0]; yield 'negative' => [-1]; yield 'very negative' => [-100]; }
}
