...
final class ClosureExportException extends ExportException
{
    ...
    public static function nestingDepthExceeded(
        int $maxDepth,
        int $depth,
        ...
        return new self(
            sprintf('Closure nesting depth %d exceeds maxDepth %d.', $depth, $maxDepth),
    ...
    public static function nonPortableExpression(
        string $reason,
        ?string $filename = null,
        ?int $line = null,
    ): self {
        return new self(
            sprintf('Closure is not portable as a standalone PHP expression: %s.', $reason),
            ['reason' => $reason, 'filename' => $filename, 'line' => $line],
            $filename,
            $line,
        );
    }
